...
% ==========================================
% SECTION 4: VELOCITY VECTORS & PLOTTING
% ==========================================
Scale = 1/50; % Scale factor from your PDF
...
% 2. Plot Mechanism (Black/Grey lines for links)
plot([0 real(R_Green)], [0 imag(R_Green)], 'k-');
plot([real(R_Green) real(R_Grey)+real(RO4O2)], [imag(R_Green) imag(R_Grey)+imag(RO4O2)], 'k-');
plot([real(RO4O2) real(R_Grey)+real(RO4O2)], [imag(RO4O2) imag(R_Grey)+imag(RO4O2)], 'k-');
% (Add other loops plotting here as needed, simplified for clarity)
...
% 4. Plot Quivers (Breaking into Real/Imag components)
% Green Tip Velocity
quiver(real(R_Green), imag(R_Green), real(V_Green)/50, imag(V_Green)/50, 0, 'Color', 'g', 'LineWidth', 2);
% Grey Tip Velocity
quiver(real(R_Grey)+real(RO4O2), imag(R_Grey)+imag(RO4O2), real(V_Grey)/50, imag(V_Grey)/50, 0, 'Color', [0.5 0.5 0.5], 'LineWidth', 2);
% Yellow Relative Velocity (Plotted at Green Tip)
quiver(real(R_Green), imag(R_Green), real(V_Yellow_Rel)/50, imag(V_Yellow_Rel)/50, 0, 'Color', 'y', 'LineWidth', 2);

% Loop 2 Velocities
quiver(real(R_Cyan)+real(RO4O2), imag(R_Cyan)+imag(RO4O2), real(V_Cyan)/50, imag(V_Cyan)/50, 0, 'c', 'LineWidth', 2);
% Loop 3 Velocities
quiver(real(R_Brown)+real(RO4O2), imag(R_Brown)+imag(RO4O2), real(V_Brown)/50, imag(V_Brown)/50, 0, 'Color', [0.8 0.4 0], 'LineWidth', 2);
...
% 3. Plot Quivers
quiver(real(R_Green_2), imag(R_Green_2), real(V_Green_2)/50, imag(V_Green_2)/50, 0, 'g', 'LineWidth', 2);
quiver(real(R_Grey_2)+real(RO4O2), imag(R_Grey_2)+imag(RO4O2), real(V_Grey_2)/50, imag(V_Grey_2)/50, 0, 'k', 'LineWidth', 2);
quiver(real(R_Green_2), imag(R_Green_2), real(V_Yellow_Rel_2)/50, imag(V_Yellow_Rel_2)/50, 0, 'y', 'LineWidth', 2);
quiver(real(R_Cyan_2)+real(RO4O2), imag(R_Cyan_2)+imag(RO4O2), real(V_Cyan_2)/50, imag(V_Cyan_2)/50, 0, 'c', 'LineWidth', 2);
quiver(real(R_Brown_2)+real(RO4O2), imag(R_Brown_2)+imag(RO4O2), real(V_Brown_2)/50, imag(V_Brown_2)/50, 0, 'Color', [0.8 0.4 0], 'LineWidth', 2);

axis equal; grid on;

% Display Results
disp('--- VELOCITY RESULTS (rad/s) ---');
disp(['w_Yellow: ', num2str(w_Yellow)]);
disp(['w_Green (Open): ', num2str(w_Green_1)]);
disp(['w_Grey (Open): ', num2str(w_Grey_1)]);

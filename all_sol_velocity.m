...
% ==========================================
% SECTION 2: POSITION ANALYSIS (Freudenstein Method)
% ==========================================
...
% ==========================================
% SECTION 3: VELOCITY ANALYSIS (Angular)
% ==========================================
% w_Yellow is defined in Section 1

% --- LOOP 1 Angular Velocities ---
a = L2_Loop1; b = L3_Loop1; c = L4_Shared;
% Case 1
...
plot([real(R_Green) real(R_Grey)+real(RO4O2)], [imag(R_Green) imag(R_Grey)+imag(RO4O2)], 'y-', 'LineWidth', 2);
plot([real(RO4O2) real(R_Grey)+real(RO4O2)], [imag(RO4O2) imag(R_Grey)+imag(RO4O2)], 'Color', [0.5 0.5 0.5], 'LineWidth', 2);
% Loop 2
plot([real(RO4O2) real(R_Cyan)+real(RO4O2)], [imag(RO4O2) imag(R_Cyan)+imag(RO4O2)], 'c-', 'LineWidth', 2);
plot([real(R_Cyan)+real(RO4O2) real(R_Grey)+real(RO4O2)], [imag(R_Cyan)+imag(RO4O2) imag(R_Grey)+imag(RO4O2)], 'r-', 'LineWidth', 2);
% Loop 3
plot([real(RO4O2) real(R_Cyan_Up)+real(RO4O2)], [imag(RO4O2) imag(R_Cyan_Up)+imag(RO4O2)], 'c:', 'LineWidth', 2);
plot([real(R_Cyan_Up)+real(RO4O2) real(R_Brown)+real(RO4O2)], [imag(R_Cyan_Up)+imag(RO4O2) imag(R_Brown)+imag(RO4O2)], 'b-', 'LineWidth', 2);
plot([real(RO4O2) real(R_Brown)+real(RO4O2)], [imag(RO4O2) imag(R_Brown)+imag(RO4O2)], 'Color', [0.6 0.3 0], 'LineWidth', 2);

% 3. Calculate Velocity Vectors (V = 1i * R * w)
...
% Green Velocity (at Tip of Green)
...
% Grey Velocity (at Tip of Grey)
quiver(real(R_Grey)+real(RO4O2), imag(R_Grey)+imag(RO4O2), real(V_Grey)*Scale, imag(V_Grey)*Scale, 0, 'Color', 'k', 'LineWidth', 1.5, 'MaxHeadSize', 0.5);
% Cyan Velocity (at Tip of Cyan Down)
quiver(real(R_Cyan)+real(RO4O2), imag(R_Cyan)+imag(RO4O2), real(V_Cyan)*Scale, imag(V_Cyan)*Scale, 0, 'Color', 'c', 'LineWidth', 1.5, 'MaxHeadSize', 0.5);
% Brown Velocity (at Tip of Brown)
quiver(real(R_Brown)+real(RO4O2), imag(R_Brown)+imag(RO4O2), real(V_Brown)*Scale, imag(V_Brown)*Scale, 0, 'Color', [0.6 0.3 0], 'LineWidth', 1.5, 'MaxHeadSize', 0.5);

axis equal; grid on;

% --- CASE 2 (CROSSED) PLOTTING ---
figure(2)
hold on; title('Case 2: Crossed Circuit (Position & Velocity)');

% 1. Create Position Vectors

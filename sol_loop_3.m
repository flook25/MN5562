...
% --- 2. Define K Constants (Standard Forward Analysis: Theta 2 is Input) ---
% Constants K1-K5 for finding Theta 4 and Theta 3 given Theta 2
K1 = d/a;
K2 = d/c;
...
% --- Calculate Coefficients for Theta 4 (Rocker - Brown) ---
% A*cos(q4) + B*sin(q4) + C = 0
A1 = cos(q2_1) - K1 - K2*cos(q2_1) + K3;
B1 = -2*sin(q2_1);
C1 = K1 - (K2+1)*cos(q2_1) + K3;

% --- Calculate Coefficients for Theta 3 (Coupler - Blue) ---
% D*cos(q3) + E*sin(q3) + F = 0
D1 = cos(q2_1) - K1 + K4*cos(q2_1) + K5;
E1 = -2*sin(q2_1);
F1 = K1 + (K4-1)*cos(q2_1) + K5;

% --- Solve for Theta 4 & 3 (Case 1) ---
% Selecting the root corresponding to Open Configuration
...
% Selecting the root corresponding to Crossed Configuration
...
% --- Plotting Case 1 ---
figure(1)
title('Case 1: Open Circuit (Input q2 = -17.944)');
hold on;
% Ground (Link 1 - Pink)
...
% --- Plotting Case 2 ---
figure(2)
title('Case 2: Crossed Circuit (Input q2 = -92.2333)');
hold on;
% Ground (Link 1 - Pink)
quiver(0,0, RO4O2x, RO4O2y, 0, 'Color', [1 0 1], 'MaxHeadSize', 0.5, 'LineWidth', 4); 
% Crank (Link 2 - Cyan)
...
% Display Results
disp('--- Results ---');
disp(['Case 1 (Open): Theta 3 = ', num2str(q3_open_d), ', Theta 4 = ', num2str(q4_open_d)]);
disp(['Case 2 (Crossed): Theta 3 = ', num2str(q3_cross_d), ', Theta 4 = ', num2str(q4_cross_d)]);
